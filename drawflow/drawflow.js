...
export default class Drawflow {
   ...
   * @param {Object} render - 渲染引擎(可选)
   * @param {Object} parent - 父级对象(可选) 
   */
   ...
   * @param {number} num_in - 输入端口数量
   * @param {number} num_out - 输出端口数量
   * @param {number} pos_x - X坐标位置
   * @param {number} pos_y - Y坐标位置
   * @param {string} classoverride - 自定义CSS类
   * @param {Object} data - 节点数据
   * @param {string} html - 节点HTML内容
   * @param {boolean} typenode - 节点类型
   * @returns {string} 节点ID
   */
  addNode(name, num_in, num_out, pos_x, pos_y, classoverride, data, html, typenode = false) {
    if (this.useuuid) {
